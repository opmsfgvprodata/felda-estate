...
namespace MVC_SYSTEM.Class
{
    public class GetConfig
    {
        ...
        public List<MasterModels.tblOptionConfigsWeb> GetWebConfigList(string[] flag1, int? NegaraID, int? SyarikatID)
        {
        ...
        {
            decimal? upah = db.tbl_UpahMenuai.Where(x => x.fld_Jadual == "A" && Hasil >= x.fld_HasilLower && Hasil <= x.fld_HasilUpper && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID).Select(s => s.fld_Kadar).FirstOrDefault();
            return upah;
        }

        public decimal? UpahMesin(decimal Hasil, int? NegaraID, int? SyarikatID)
        {
            decimal? upah = db.tbl_UpahMenuai.Where(x => x.fld_Jadual == "B" && Hasil >= x.fld_HasilLower && Hasil <= x.fld_HasilUpper && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID).Select(s => s.fld_Kadar).FirstOrDefault();
            return upah;
        }
        ...
        public string GetGroupDescFromID(int? id, int? NegaraID, int? SyarikatID, int? WilayahID, int? LadangID, string host, string user, string catalog, string pass)
        {
            MVC_SYSTEM_Models dbr = MVC_SYSTEM_Models.ConnectToSqlServer(host, catalog, user, pass);

            var groupData = dbr.tbl_KumpulanKerja.SingleOrDefault(x => x.fld_KumpulanID == id);

            return groupData.fld_Keterangan;
        }

        public string GetGroupCodeFromID(int? id, int? NegaraID, int? SyarikatID, int? WilayahID, int? LadangID, string host, string user, string catalog, string pass)
        {
            MVC_SYSTEM_Models dbr = MVC_SYSTEM_Models.ConnectToSqlServer(host, catalog, user, pass);

            var groupData = dbr.tbl_KumpulanKerja.SingleOrDefault(x => x.fld_KumpulanID == id);

            return groupData.fld_KodKumpulan;
        }

        public string GetGroupAcitivityCodeFromID(int? id, int? NegaraID, int? SyarikatID, int? WilayahID, int? LadangID, string host, string user, string catalog, string pass)
        {
            MVC_SYSTEM_Models dbr = MVC_SYSTEM_Models.ConnectToSqlServer(host, catalog, user, pass);

            var groupData = dbr.tbl_KumpulanKerja.SingleOrDefault(x => x.fld_KumpulanID == id);

            return groupData.fld_KodKerja;
        }

        public string GetPktName(string kodPkt, int? NegaraID, int? SyarikatID, int? WilayahID, int? LadangID)
        {
            ...
            var pkt = dbr.tbl_PktUtama
                .SingleOrDefault(x =>
                    x.fld_PktUtama == kodPkt && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID &&
                    x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID);

            if (pkt != null)
            {
                pktName = pkt.fld_NamaPktUtama;
            }

            return pktName;
        }

        public string GetSubPktName(string kodSubPkt, int? NegaraID, int? SyarikatID, int? WilayahID, int? LadangID)
        {
            string host, catalog, user, pass = "";
            Connection.GetConnection(out host, out catalog, out user, out pass, WilayahID.Value, SyarikatID.Value, NegaraID.Value);
            MVC_SYSTEM_Models dbr = MVC_SYSTEM_Models.ConnectToSqlServer(host, catalog, user, pass);
            ...
            var subPkt = dbr.tbl_SubPkt
                .SingleOrDefault(x =>
                    x.fld_Pkt == kodSubPkt && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID &&
                    x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID);

            if (subPkt != null)
            {
                subPktName = subPkt.fld_NamaPkt;
            }

            return subPktName;
        }

        public string GetBlokName(string kodBlok, int? NegaraID, int? SyarikatID, int? WilayahID, int? LadangID)
        {
            string host, catalog, user, pass = "";
            Connection.GetConnection(out host, out catalog, out user, out pass, WilayahID.Value, SyarikatID.Value, NegaraID.Value);
            MVC_SYSTEM_Models dbr = MVC_SYSTEM_Models.ConnectToSqlServer(host, catalog, user, pass);
            ...
            var Blok = dbr.tbl_Blok
                .SingleOrDefault(x =>
                    x.fld_Blok == kodBlok && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID &&
                    x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID);

            if (Blok != null)
            {
                BlokName = Blok.fld_NamaBlok;
            }
        ...
        public int GetKerjaHadirCount(string KodKumpulan, int? NegaraID, int? SyarikatID, int? WilayahID, int? LadangID)
        {
            ...
            var currentYear = changeTimeZone.gettimezone().Year;

            var kerjaHadirCount = dbr.tbl_Kerjahdr
                .Count(x => x.fld_Kum == KodKumpulan && x.fld_Tarikh.Value.Month == currentMonth && x.fld_Tarikh.Value.Year == currentYear && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID &&
                            x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID);
        ...
        public int GetKerjaPktCount(string KodPkt, int? NegaraID, int? SyarikatID, int? WilayahID, int? LadangID)
        {
            string host, catalog, user, pass = "";
            Connection.GetConnection(out host, out catalog, out user, out pass, WilayahID.Value, SyarikatID.Value, NegaraID.Value);
            MVC_SYSTEM_Models dbr = MVC_SYSTEM_Models.ConnectToSqlServer(host, catalog, user, pass);
            ...
            var kerjaPktCount = dbr.tbl_Kerja
                .Count(x => x.fld_KodPkt == KodPkt && x.fld_Tarikh.Value.Month == currentMonth && x.fld_Tarikh.Value.Year == currentYear && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID &&
                            x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID);
