    ...
    public class SendEmail
    {
        ...
        public bool SendEmailLatest(string subject, string msg, string[] to, string[] cc, string[] bcc)
        {
            ...
            try
            {
                ...
                SmtpServer.UseDefaultCredentials = getemailcrdnt();
                SmtpServer.EnableSsl = getemailssl();
        ...
        // Sepul tambah 01/04/2021 emel reset password
        public bool SendEmailForgotPassword(string subject, string msg, string to)
        {
            string[] multirecpt = new string[] { };
            bool result = false;
            try
            {
                ...
                mail.From = new MailAddress(getemailfrom(), "OPMS - KATALALUAN");
                if (to != null && to != "")
                {
                    mail.To.Add(to);
                }
        ...
        private bool getemailssl()
        {
            bool getresult = true;
            IniParser parser = new IniParser(AppDomain.CurrentDomain.BaseDirectory + "config.ini");

            getresult = bool.Parse(parser.GetSetting("configdeclaration", "emailssl"));

            return getresult;
        }
        private bool getemailcrdnt()
        {
            bool getresult = true;
            IniParser parser = new IniParser(AppDomain.CurrentDomain.BaseDirectory + "config.ini");

            getresult = bool.Parse(parser.GetSetting("configdeclaration", "emailcrdnt"));
