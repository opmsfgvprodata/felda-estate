...
//GL TO VENDOR
        ...
        public ActionResult CloseTransaction(int Month, int Year, bool CloseOpen)
        {
            ...
            {
                var kerjaDate = dbr.tbl_Kerja.Where(x => x.fld_LadangID == LadangID && x.fld_WilayahID == WilayahID && x.fld_Tarikh.Value.Month == Month && x.fld_Tarikh.Value.Year == Year).OrderByDescending(x => x.fld_CreatedDT).FirstOrDefault();
                DateTime? KerjaTime = kerjaDate== null? null : kerjaDate.fld_CreatedDT;
                DateTime? JanaTime = db.tbl_SevicesProcess.Where(x => x.fld_Year == Year && x.fld_Month == Month && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Flag==1).OrderByDescending(x => x.fld_DTProcess).Select(x=>x.fld_DTProcess).FirstOrDefault();
                if (JanaTime == null)
                {
                    JanaTime = db.tbl_SevicesProcessHistory.Where(x => x.fld_Year == Year && x.fld_Month == Month && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Flag==1).OrderByDescending(x=>x.fld_DTProcess).Select(x=>x.fld_DTProcess).FirstOrDefault();
                }
        
                if ((JanaTime > KerjaTime && CloseOpen == true) || CloseOpen == false)
                {
