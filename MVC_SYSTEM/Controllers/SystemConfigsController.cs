        ...
        public ActionResult Edit(int id,[Bind(Include = "fldConfigID,fldConfigValue,fldConfigDesc,fldFlag1,fldFlag2,fldDeleted")] MasterModels.tblSystemConfig tblSystemConfig)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    db.SaveChanges();
                    var getid = id;
                    return Json(new { success = true, msg = GlobalResEstate.msgUpdate, status = "success", checkingdata = "0", method = "1", getid = getid, data1 = tblSystemConfig.fldConfigDesc, data2 = tblSystemConfig.fldConfigValue });
                }
                catch (Exception ex)
                {
            ...
            {
                return Json(new { success = true, msg = GlobalResEstate.msgDataFill, status = "warning", checkingdata = "1" });
            }
            //return View(tblSystemConfig);
        }
