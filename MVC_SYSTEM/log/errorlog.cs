...
namespace MVC_SYSTEM.log
{
    public class errorlog
    {
        ...
        public void testservedoing(string data1)
        {
            string path = HttpContext.Current.Server.MapPath("~/TestLog/testservedoing-" + timezone.gettimezone().ToString("dd-MM-yyyy") + ".txt");
            using (StreamWriter writer = new StreamWriter(path, true))
            {
